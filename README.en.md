...
- Automated releases attach macOS downloads as versioned zip files:
  - `D-academic-agent-v<version>-mac-arm64.zip` for Apple Silicon Macs.
  - `D-academic-agent-v<version>-mac-x64.zip` for Intel Macs.
  - `D-academic-agent-v<version>-mac-universal.zip` for one app that supports both.

Older releases may have fewer assets.

The macOS app is not notarized yet. If macOS blocks it as an app from an unidentified developer, right-click the app,
choose **Open**, then confirm. Windows and Linux users should run from source for now.
...
Use `npm run package` when you need a local macOS `.app` directory build. Tagged releases matching `v*.*.*`
automatically build and upload macOS `arm64`, `x64`, and `universal` zip assets through GitHub Actions.

Release tags must match `package.json`, for example `package.json` version `0.1.1` should be tagged as `v0.1.1`.
